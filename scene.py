...
class Scene:
    ...
    def commit(self):
        ...
        boxs_type = ti.types.struct(
            min=vec3f, box_max=vec3f, pos=vec3i, texture_pro=vec3i, has_texture=ti.i32, id=ti.i32, area=ti.f32,
            geo_type=ti.i32, li_type=ti.i32
        )
        ...
        if texIdx > 0:
            ...
            self.texture_datas = ti.Vector.field(n=3, dtype=ti.i32, shape=(1))
    ...
    def getTextureColor(self, obj, p, tri_index):
        ...
        c_idx = ti.cast((1 - tx.y) * theight, dtype=ti.i32) * tweight + ti.cast((tx.x) * tweight, dtype=ti.i32) - 1 + \
                obj.texture_pro[2]
    ...
    def hit_obj(self, obj_id, ray_origin, ray_direction, t_min, closest_so_far):
        ...
        if obj_box.geo_type == 1:
            hit_anything, t, p, n, front_facing, hit_tri_index = self.hit_meshs(obj_box, ray_origin, ray_direction,
                                                                                t_min, closest_so_far)

        if obj_box.geo_type == 2:
            hit_anything, t, p, n, front_facing = self.hit_sphere(obj_box, ray_origin, ray_direction, t_min,
                                                                  closest_so_far)
    ...
    def hit_light(self, ray_origin, out_dir, obj):
        ...
        is_hit = True
        if obj.geo_type == 1:
            is_hit, t, coords, outward_normal, front_facing, hit_tri_index = self.hit_meshs(obj, ray_origin, out_dir,
                                                                                            t_min, t_max)
